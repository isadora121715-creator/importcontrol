...
const Embarques = () => {
  ...
  const intlKpis = useMemo(() => {
    ...
    // Fallback para totais oficiais apenas quando não há filtro E cols Col8/9/10 vieram vazias
    const containersFinal = (computedTotal === 0) ? {
      "20ft": intlTotals.cont20,
      "40ft": intlTotals.cont40,
      "45ft": intlTotals.cont45,
    } : containers;
    const modalidadesFinal = !hasFilter ? {
      LCL: modalidades.LCL,
      FCL: modalidades.FCL,
      Aereo: intlTotals.aereo || modalidades.Aereo,
    } : modalidades;
  ...
  interface VendaItem {
    ...
    qtd: string;
    freteUSD: string;       // frete total em USD para este item
    impostoPct: string;     // impostos / despesas (% sobre custo)
    cambio: string;
    precoVendaBRL: string;  // preço de venda definido pelo usuário (opcional)
  }
  ...
  const [vendaForm, setVendaForm] = useState<Omit<VendaItem, "id">>({
    descricao: "", precoCompraUSD: "", qtd: "1",
    freteUSD: "", impostoPct: "", cambio: "5.20", precoVendaBRL: "",
  });

  const saveVendaItems = (items: VendaItem[]) => {
    setVendaItems(items);
  ...
  const addVendaItem = () => {
    ...
    const novo: VendaItem = { ...vendaForm, id: Date.now().toString() };
    saveVendaItems([...vendaItems, novo]);
    setVendaForm({ descricao: "", precoCompraUSD: "", qtd: "1", freteUSD: "", impostoPct: "", cambio: "5.20", precoVendaBRL: "" });
  };

  const removeVendaItem = (id: string) => saveVendaItems(vendaItems.filter((i) => i.id !== id));

  const calcVenda = (item: VendaItem, margemPct: number) => {
    ...
    // Custo unitário em BRL (com impostos/despesas)
    const custoUnitBRL = custoUnitUSD * cambio * (1 + imposto / 100);
    // Preço mínimo de venda (margem sobre preço de venda)
    const vendaMinBRL  = custoUnitBRL / (1 - margemPct / 100);
    // Margem real se o usuário definiu preço
    ...
    return { qtd, custoUnitUSD, custoUnitBRL, custoTotalBRL: custoUnitBRL * qtd, vendaMinBRL, margemReal, abaixoMinimo };
  };
                ...
                <div className="rounded-lg border p-4 space-y-3 bg-muted/30">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="md:col-span-2">
                      ...
                      <Input placeholder="Ex: Válvula DN50 PN16"
                    ...
                    <div>
                      <label className="text-xs text-muted-foreground mb-1 block">Preço Venda (R$/un) <span className="text-[10px]">opcional</span></label>
                      <Input type="number" min="0" placeholder="deixe vazio para calcular"
                        value={vendaForm.precoVendaBRL}
                        onChange={(e) => setVendaForm((f) => ({ ...f, precoVendaBRL: e.target.value }))}
                      />
                    </div>
                  </div>
                  <Button onClick={addVendaItem} disabled={!vendaForm.descricao || !vendaForm.precoCompraUSD}>
                    + Adicionar Item
                  </Button>
                </div>
                ...
                {vendaItems.length > 0 && (
                  <div className="overflow-x-auto rounded-lg border">
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="bg-muted border-b">
                          {["Descrição","Qtd","Custo Unit (USD)","Frete/Un (USD)","Custo Unit (R$)","Custo Total (R$)","Venda Mín (R$/un)","Preço Venda (R$/un)","Margem Real",""].map((h) => (
                      ...
                      <tbody>
                        {vendaItems.map((item) => {
                          const c = calcVenda(item, Number(vendaMargem) || 18);
                          return (
                            <tr key={item.id} className={cn("border-b last:border-0 hover:bg-muted/30", c.abaixoMinimo && "bg-red-50 dark:bg-red-950/20")}>
                              ...
                              <td className="px-3 py-2">$ {(Number(item.freteUSD) / c.qtd || 0).toFixed(2)}</td>
                              <td className="px-3 py-2">R$ {c.custoUnitBRL.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</td>
                              <td className="px-3 py-2">R$ {c.custoTotalBRL.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</td>
                              <td className="px-3 py-2 font-semibold text-primary">
                                R$ {c.vendaMinBRL.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                              </td>
                              <td className="px-3 py-2">
                                {item.precoVendaBRL ? (
                                  <span className={cn("font-semibold", c.abaixoMinimo ? "text-destructive" : "text-green-600 dark:text-green-400")}>
                                    R$ {Number(item.precoVendaBRL).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                                    {c.abaixoMinimo && " ⚠"}
                                  </span>
                              ...
                              <td className="px-3 py-2">
                                {c.margemReal !== null ? (
                                  <span className={cn("font-bold", c.margemReal >= Number(vendaMargem) ? "text-green-600 dark:text-green-400" : "text-destructive")}>
                                    {c.margemReal.toFixed(1)}%
                                  </span>
                                ) : <span className="text-muted-foreground">—</span>}
                              </td>
                              <td className="px-3 py-2">
                                <Button size="sm" variant="ghost" onClick={() => removeVendaItem(item.id)}>
                      ...
                      <tfoot>
                        <tr className="bg-muted/50 font-semibold text-xs border-t">
                          ...
                          <td className="px-3 py-2">
                            R$ {vendaItems.reduce((acc, item) => acc + calcVenda(item, Number(vendaMargem) || 18).custoTotalBRL, 0)
                              .toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                          </td>
                          <td className="px-3 py-2 text-primary">
                            R$ {vendaItems.reduce((acc, item) => acc + calcVenda(item, Number(vendaMargem) || 18).vendaMinBRL * calcVenda(item, Number(vendaMargem) || 18).qtd, 0)
                              .toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                          </td>
                          <td colSpan={3} />
                        </tr>

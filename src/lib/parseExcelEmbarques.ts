...
const EMBARQUES_COL_MAP: Record<string, keyof PedidoRow> = {
...
export function parseExcelEmbarques(file: File): Promise<PedidoRow[]> {
      ...
      try {
        ...
        for (let r = headerRowIndex + 1; r <= range.e.r; r++) {
          ...
          for (let c = range.s.c; c <= maxCol; c++) {
            ...
            const field = EMBARQUES_COL_MAP[norm];
            if (field) {

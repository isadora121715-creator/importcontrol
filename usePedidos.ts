...
// ── Cross-user sync (layer 2) ────────────────────────────────────────────────
// Live sync using a public Supabase Storage file (see src/lib/jsonSync.ts) —
// any browser can read/write it directly, no token or credential involved.
// This replaces the previous GitHub Gist sync, which required an access
// token embedded in this file (a security exposure fixed 2026-09).
const SYNC_FILE: Record<string, string> = {
...
async function fetchAllPedidos(categoria: string): Promise<PedidoRow[]> {
  ...
  // 2️⃣ Shared Supabase Storage snapshot — the latest version uploaded by any
  //    user, picked up immediately on every other device.
  const syncFile = SYNC_FILE[categoria];
  if (syncFile) {
    const data = await fetchJson<PedidoRow[]>(syncFile);
...
export function usePedidos(categoria: string = "Conexões") {
  ...
  const query = useQuery({
    ...
    refetchInterval: 60_000,     // poll every minute so other users' updates show up live
    enabled: !isUpdating,
  });
  ...
  const handleFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 🔄 Shared sync — write the JSON to Supabase Storage so every other
      //    browser reads the update immediately (no auth needed to read).
      const syncFile = SYNC_FILE[categoria];
      if (syncFile) {
        try {
          setUpdateMessage("Sincronizando com outros usuários...");
          await uploadJson(syncFile, rows);
        } catch (e) {
          console.warn("Sync error (not critical):", e);
        }
      }
